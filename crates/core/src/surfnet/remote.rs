...
impl SurfnetRemoteClient {
    ...
    // The raw client error can contain the secret in the datasource URL, so it is cleaned up.
    pub async fn get_epoch_schedule(&self) -> SurfpoolResult<EpochSchedule> {
        self.client.get_epoch_schedule().await.map_err(|error| {
            SurfpoolError::get_epoch_schedule(sanitized_client_error(&error, &self.client.url()))
        })
    }

    // The raw client error can contain the secret in the datasource URL, so it is cleaned up.
    pub async fn get_slot(&self, commitment_config: CommitmentConfig) -> SurfpoolResult<Slot> {
        self.client
            .get_slot_with_commitment(commitment_config)
            .await
            .map_err(|error| {
                SurfpoolError::get_slot(sanitized_client_error(&error, &self.client.url()))
            })
    }
...
mod tests {
    ...
    /// The raw client error can contain the secret in the datasource URL.
    #[tokio::test]
    async fn a_slot_or_epoch_schedule_failure_does_not_pass_the_raw_client_error_through() {
        let client = client_from(Fails("SUPERSECRET".to_string()));

        let messages = [
            client
                .get_slot(CommitmentConfig::confirmed())
                .await
                .expect_err("a datasource that refuses the request should not succeed")
                .to_string(),
            client
