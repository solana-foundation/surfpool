...
/// Re-executes a remote transaction against a fresh surfnet whose clock is set to its original
/// slot. The accounts it runs against are read from the datasource at its current slot.
pub async fn handle_profile_command(cmd: ProfileCommand, _ctx: &Context) -> Result<(), String> {
    ...
    let epoch_schedule = remote_client
        .get_epoch_schedule()
        .await
        .map_err(|e| e.to_string())?;
    ...
    // Accounts are only fetched once the transaction runs, so this is the slot just before they
    // are read rather than the exact one each was read at.
    let current_slot = remote_client
        .get_slot(commitment)
        .await
        .map_err(|e| e.to_string())?;

    println!(
        "Re-executing with the clock at slot {} (epoch {}){}, against accounts read at the current slot ({})",
        slot,
        epoch_info.epoch,
        block_time
    ...
    let remote_ctx = Some((remote_client, commitment));
